...
export async function initDatabase() {
  ...
  try {
    ...
    try {
      await db.execute(`ALTER TABLE projects ADD COLUMN users TEXT`);
    } catch {
      // Column already exists
    }
    ...
    // Add jira_url column for JIRA ticket links (migration)
    try {
      await db.execute(`
    ...
    // Add sort_order column for drag & drop sorting (migration)
    try {
      await db.execute(`
    ...
    // Add metadata column for flexible browser/system info storage (migration)
    try {
      await db.execute(`
    ...
    // Add AI analysis columns to tasks table (migration)
    try {
      await db.execute(`
        ALTER TABLE tasks ADD COLUMN ai_sentiment TEXT
      `);
    } catch (error) {
      if (!error.message.includes('duplicate column name')) {
      }
    ...
    try {
      await db.execute(`
        ALTER TABLE tasks ADD COLUMN ai_confidence INTEGER
      `);
    } catch (error) {
      if (!error.message.includes('duplicate column name')) {
      }
    ...
    try {
      await db.execute(`
        ALTER TABLE tasks ADD COLUMN ai_category TEXT
      `);
    } catch (error) {
      if (!error.message.includes('duplicate column name')) {
      }
    ...
    try {
      await db.execute(`
        ALTER TABLE tasks ADD COLUMN ai_keywords TEXT
      `);
    } catch (error) {
      if (!error.message.includes('duplicate column name')) {
      }
    ...
    try {
      await db.execute(`
        ALTER TABLE tasks ADD COLUMN ai_analyzed_at DATETIME
      `);
    } catch (error) {
      if (!error.message.includes('duplicate column name')) {
      }
    ...
    // Authorized Users-Tabelle für Magic Link Auth
    await db.execute(`
    ...
    // Magic Tokens-Tabelle für passwordless login
    await db.execute(`
    ...
    // User Project Access-Tabelle für Non-Admin Users
    await db.execute(`
